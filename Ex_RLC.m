...
% impedance spectra related to an RLC circuit. The circuit is constituted
% by two RC units, an RL unit and a resistor connected in parallel.
...
% -Creating an impedance data set related to the RLC circuit <=> (Step 1)
...
Z_RLC(i)=1/((1/R1)+1j*w*C1) +1/((1/R2)+1j*w*C2) +1/((1/R3)+(1/(1j*w*L3))) ...
        +R0;
...
% entries
[L,Ls,V,W] = Loewner_Framework(1i*2*pi*f,Z_RLC,'real');
...
ReZ_rel=abs(((real(Z_RLC)-real(Hk))./(abs(Z_RLC))))*100;
ImZ_rel=abs(((imag(Z_RLC)-imag(Hk))./(abs(Z_RLC))))*100;
...
plot(real(Z_RLC),-imag(Z_RLC),'ok','markersize',5);
hold on
